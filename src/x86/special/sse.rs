...
#[assign_targets(Add, Sub, Mul, Div)]
#[assign_rhs(Self, f32)]
pub struct f32x3 (pub(crate) __m128);

impl f32x3 {
    const DIV_MASK : __m128 = unsafe { transmute([u32::MAX, u32::MAX, u32::MAX, 0]) };
    const ABS_MASK : __m128 = unsafe { transmute([i32::MAX, i32::MAX, i32::MAX, 0]) };

    const MIN_MASK : __m128 = unsafe { transmute([0., 0., 0., f32::MAX]) };
    const MAX_MASK : __m128 = unsafe { transmute([0., 0., 0., f32::MIN]) };

    /// Loads values from the pointer into the SIMD vector
    #[inline(always)]
    pub unsafe fn load (ptr: *const f32) -> Self {
        Self(_mm_set_ps(0., *ptr.add(2), *ptr.add(1), *ptr))
    }
    ...
    #[inline(always)]
    pub fn min (self) -> f32 {
        unsafe { f32x4(_mm_or_ps(Self::MIN_MASK, self.0)).min() }
    }

    /// Gets the biggest/maximum value of the vector
    #[inline(always)]
    pub fn max (self) -> f32 {
        unsafe { f32x4(_mm_or_ps(Self::MAX_MASK, self.0)).max() }
    }
...
impl PartialEq for f32x3 {
    ...
    fn eq (&self, other: &Self) -> bool {
        unsafe {
            let cmp = _mm_cmpeq_ps(self.0, other.0);
            transmute::<__m128, u128>(cmp) == u128::MAX
        }
    }
}
